...
![A project overlay with two sessions, one selected update, and a named project tab.](extension/images/screenshots/overlay-project-groups.png)
...
| Move a border tab | Drag its folded tab, or the header or background of its popped-out preview, up or down. It stays snapped to the edge; nearby tabs make room. Positions are saved per project and overlay corner across helper restarts and updates. Tabs return to their saved positions after hiding or folding. |
...
| Start a new chat | Click the **chat-plus icon** in the header of either the expanded drawer or full-height side panel. A blank session opens in the same overlay and project, with the message box focused. Existing drafts stay with their sessions. |
...
| Read the full chat beside other apps | Double-click its tab, preview, or title; or keep composing for five seconds. The chat uses the right third of the monitor, leaving two-thirds for other apps. **Escape**, the header arrow, or clicking the background again folds the chat and restores the previous window sizes. |
| Open a chat | Choose **Open chat** or use the keyboard open shortcut. Double-click a session title to read its conversation in the side panel. Editor chats open in VS Code; overlay-created chats expand in the same overlay. Choose **Open in VS Code** below an overlay-created conversation to open its saved history in the original project. |
...
[MIT licensed](LICENSE). See the [changelog](CHANGELOG.md) for updates. Screenshots show the native demo with sample chat text. Bundled sounds are from Herdr 0.8.2 under Apache-2.0; see the [third-party notices](extension/THIRD_PARTY_NOTICES.md).
